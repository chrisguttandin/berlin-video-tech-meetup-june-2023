...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/berlin-video-tech-meetup-june-2023/index.html': ['build/berlin-video-tech-meetup-june-2023/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/berlin-video-tech-meetup-june-2023/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/berlin-video-tech-meetup-june-2023/ngsw.json']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, extension) =>
                            grunt.file.expand({ cwd: 'build/berlin-video-tech-meetup-june-2023', ext: extension }, `assets/${filename}.*`)[0]
                    },
                    {
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/berlin-video-tech-meetup-june-2023/${filename}.html": "${computeHashOfFile(
                                `build/berlin-video-tech-meetup-june-2023/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/berlin-video-tech-meetup-june-2023/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/berlin-video-tech-meetup-june-2023/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(
                                `build/berlin-video-tech-meetup-june-2023/${filename}`
                            )}</script>`;
                        }
                    }
                ]
